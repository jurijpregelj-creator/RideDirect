...
export default function LoginPage() {
  ...
  const isFunnelSubmit = nextParam.includes("submitted=1")

  // Default redirect: funnel submit always lands on user dashboard,
  // otherwise admins go to /admin, regular users to /dashboard
  function getDefaultRedirect(isAdmin: boolean): string {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (error) {
      ...
      const isAdmin = ADMIN_EMAILS.includes(loggedIn?.email ?? "")
      window.location.href = getDefaultRedirect(isAdmin)
    }
  }
  ...
  function buildOAuthRedirect(): string {
    ...
    if (nextParam) return `${callbackBase}?next=${encodeURIComponent(nextParam)}`
    return callbackBase
  }
