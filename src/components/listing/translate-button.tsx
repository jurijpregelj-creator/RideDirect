...
export function TranslateButton({ originalText, onTranslated }: TranslateButtonProps) {
  ...
  // Don't show button if browser lang matches detected source lang
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {!translated ? (
        <button
          ...
          disabled={loading}
          className="flex items-center gap-1.5 text-xs text-[#1B4FD8] hover:underline disabled:opacity-50"
        >
          {loading ? (
            <><Loader2 size={13} className="animate-spin" /> Translating...</>
          ) : (
            <><Languages size={13} /> Translate to {langName}</>
          )}
        </button>
      ) : (
        <button
