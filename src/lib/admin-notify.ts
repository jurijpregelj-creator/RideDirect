...
// Constructed lazily (not at module scope) — Resend's constructor throws
// immediately on a missing key, which would crash every server action that
// imports this file the moment RESEND_API_KEY is unset (e.g. local dev),
// even ones that never send an email.
...
export async function notifyAdminNewOutreachLead(lead: NewOutreachLeadInfo) {
  const resend = getResend()
  if (!resend) return
  try {
    await resend.emails.send({
      from: "RideDirect <noreply@ridedirect.eu>",
      to: ADMIN_RECIPIENTS,
      subject: `[Outreach Lead] ${lead.itemDescription}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #0D2A5E;">New outreach lead responded</h2>
          <p style="color: #666;">Someone you contacted about their existing listing filled in the /list-here form.</p>
          <hr style="border: none; border-top: 1px solid #eee; margin: 24px 0;" />
          <table style="width: 100%; border-collapse: collapse;">
            <tr><td style="padding: 8px 0; color: #999; width: 120px;">Item</td><td style="padding: 8px 0; font-weight: 600;">${lead.itemDescription}</td></tr>
            <tr><td style="padding: 8px 0; color: #999;">Existing listing</td><td style="padding: 8px 0;"><a href="${lead.listingUrl}" style="color: #1E88E5;">${lead.listingUrl}</a></td></tr>
            ${lead.name ? `<tr><td style="padding: 8px 0; color: #999;">Name</td><td style="padding: 8px 0;">${lead.name}</td></tr>` : ""}
            ${lead.email ? `<tr><td style="padding: 8px 0; color: #999;">Email</td><td style="padding: 8px 0;"><a href="mailto:${lead.email}" style="color: #1E88E5;">${lead.email}</a></td></tr>` : ""}
            ${lead.phone ? `<tr><td style="padding: 8px 0; color: #999;">Phone</td><td style="padding: 8px 0;">${lead.phone}</td></tr>` : ""}
          </table>
          <hr style="border: none; border-top: 1px solid #eee; margin: 24px 0;" />
          <a href="https://ridedirect.eu/admin/leads" style="display:inline-block;background:#1E88E5;color:white;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:14px;">View in Admin</a>
        </div>
    ...
    console.error("[Email] Failed to send outreach-lead admin notification:", err)
  }
}
